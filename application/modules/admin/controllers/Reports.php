...
class Reports extends Admin_Controller {
	...
	public function index(){
		...
		$crud = $this->generate_crud('reports');
		$crud->columns('reports_name','reports_desc','supporting_document','created_at','report_type','status');

		$crud->change_field_type('created_at','hidden',date('Y-m-d H:i:s'));
		//	$crud->unset_columns('description');
		//$crud->set_relation('sub_cat','s_a_categories','sub_cat');
		//	$crud->display_as('sub_cat','Sub Category');
		...
		function callback_insert($post_array, $primary_key)
		{
		  $mdate = array(
		  	...
		  	"reports_id"=> $primary_key,
		  	"created_at" => date('Y-m-d H:i:s')
		  );

		  	$this->db->insert('reports', $mdate);
